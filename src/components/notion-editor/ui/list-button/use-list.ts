...
export function canToggleList(
	...
	if (!turnInto) {
		switch (type) {
			case "bulletList":
				return editor.can().toggleBulletList("bulletList");
			case "orderedList":
				return editor.can().toggleOrderedList("orderedList");
			case "taskList":
				return editor.can().toggleTaskList("taskList");
			default:
				return false;
		}
	}
	...
	switch (type) {
		case "bulletList":
			return editor.can().toggleBulletList("bulletList") || editor.can().clearNodes();
		case "orderedList":
			return editor.can().toggleOrderedList("orderedList") || editor.can().clearNodes();
		case "taskList":
			return editor.can().toggleTaskList("taskList") || editor.can().clearNodes();
		default:
			return false;
	}
}
	...
	try {
		...
		if (editor.isActive(type)) {
			...
			const toggleMap: Record<ListType, () => typeof chain> = {
				bulletList: () => chain.toggleBulletList("bulletList"),
				orderedList: () => chain.toggleOrderedList("orderedList"),
				taskList: () => chain.toggleTaskList("taskList"),
			};
