...
} from "../../ui/color-text-popover";
import { useColorTextPopover, useRecentColors, getColorByValue } from "../../ui/color-text-popover";
import { TEXT_COLORS, ColorTextButton, ColorItem } from "../../ui/color-text-button";
import { HIGHLIGHT_COLORS, ColorHighlightButton } from "../../ui/color-highlight-button";
...
// --- Utils ---
import { chunkArray } from "../../lib/advanced-utils";

// --- Styles ---
import "../../ui/color-text-popover/color-text-popover.scss";
...
export const RecentColorButton: React.FC<RenderColorButtonProps> = ({
	...
	return colorObj.type === "text" ? (
		...
		<ColorHighlightButton highlightColor={color.value} flag={color.flag} {...commonProps} />
	);
};
...
const RecentColorsSection: React.FC<RecentColorsSectionProps> = ({
	...
	return (
		<CardItemGroup>
			<CardGroupLabel>{t("text_style.recently_used")}</CardGroupLabel>
	...
	return (
		<Card ref={containerRef} tabIndex={0} role="menu">
			<CardBody>
				...
				<CardItemGroup>
					<CardGroupLabel>{t("text_style.text_color")}</CardGroupLabel>
					<ColorGroup
						type="text"
				...
				<CardItemGroup>
					<CardGroupLabel>{t("text_style.highlight_color")}</CardGroupLabel>
					<ColorGroup
						type="highlight"
...
export const ColorTextPopover = forwardRef<HTMLButtonElement, ColorTextPopoverProps>(
	({ hideWhenUnavailable = false, onColorChanged, onClick, children, ...buttonProps }, ref) => {
		const { t } = useLanguage();
