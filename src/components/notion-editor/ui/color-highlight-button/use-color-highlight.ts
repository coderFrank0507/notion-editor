...
// --- Lib ---
import { isMarkInSchema, isNodeTypeSelected, isExtensionAvailable } from "../../lib/utils";

// --- Icons ---
import { HighlighterIcon } from "../../icons/highlighter-icon";
import { ColorItem } from "../color-text-button";
...
export function isColorHighlightActive(
	...
	if (mode === "mark") {
		return flag ? editor.isActive("highlight", { color: flag }) : editor.isActive("highlight");
	} else {
		...
		try {
			...
			for (let depth = $pos.depth; depth >= 0; depth--) {
				...
				if (node && node.attrs?.backgroundColor === flag) {
					return true;
				}
			}
			return false;
...
export function useColorHighlight(config: UseColorHighlightConfig) {
	...
	const isActive = isColorHighlightActive(editor, flag, mode);
